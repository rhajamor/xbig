...
<xsl:stylesheet version="2.0"
	...
	<xsl:template name="cppEnum">
		...
		<xsl:variable name="class_prefix">
			<xsl:choose>
				<xsl:when test="$isInnerClass">
					<xsl:value-of select="concat($ns_prefix, '_', '00024', @name, 'Helper')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="concat($ns_prefix, '_', @name, 'Helper')"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:result-document href="{xbig:toFileURL($header_filename)}"
			...
			<xsl:text> * Method:    </xsl:text><xsl:text>getEnumValues&#10;</xsl:text>
		...
		<xsl:result-document href="{xbig:toFileURL($impl_filename)}"
			...
			<xsl:for-each select="$nodeContainingIncludeFiles/node/*/libraryfile">
				<xsl:text>&#10;#include "</xsl:text>
